...
export default function CommandPalette({ isOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-20 bg-slate-900/50 backdrop-blur-xs p-4 animate-fade-in font-sans">
      <div className="bg-white border border-slate-300 rounded-xl w-full max-w-xl shadow-2xl overflow-hidden text-xs">
        {/* Search input header */}
        <div className="p-3.5 border-b border-slate-200 flex items-center gap-3 bg-slate-50">
          ...
          <input
            ...
            placeholder="Type a command, sensor ID (e.g. NODE-01), or page name..."
            className="flex-1 text-sm text-slate-900 placeholder-slate-400 focus:outline-none bg-transparent font-medium"
          />
          <kbd className="bg-white border border-slate-300 rounded px-1.5 py-0.5 text-[10px] font-mono text-slate-600 font-bold shadow-xs">
            ESC
          </kbd>
        </div>
            ...
            <div className="space-y-1">
              {matchingPages.map((p) => (
                <button
                  ...
                  className="w-full px-2.5 py-2 rounded-md hover:bg-slate-100 flex items-center gap-3 text-left transition-colors"
                >
                  ...
                  <span className="text-xs text-[#0B6E4F] font-bold">Jump →</span>
                </button>
              ))}
            </div>
          </div>
              ...
              <div className="space-y-1">
                {matchingNodes.map((n) => (
                  <button
                    ...
                    className="w-full px-2.5 py-2 rounded-md hover:bg-slate-100 flex items-center justify-between text-left transition-colors"
                  >
                    <div>
                      <div className="font-bold text-slate-900 flex items-center gap-2">
                        <span className="font-mono">📡 {n.node_id}</span>
                        <span className="text-[11px] font-normal text-slate-600">{n.name}</span>
                      </div>
                      <div className="text-[10px] text-slate-500 truncate max-w-sm">{n.location}</div>
                    </div>
              ...
              <div className="space-y-1">
                {matchingAlerts.map((a) => (
                  <button
                    ...
                    className="w-full px-2.5 py-2 rounded-md hover:bg-red-50 flex items-center justify-between text-left transition-colors"
                  >
                    <div>
